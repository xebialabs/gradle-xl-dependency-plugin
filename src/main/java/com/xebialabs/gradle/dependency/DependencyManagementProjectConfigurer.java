  ...
  private static void enforceVersion(Project project, DependencyManagementContainer container, DependencyResolveDetails details) {
    ...
    if (version != null) {
      if (version.startsWith("${")) {
        ...
        project.getLogger().info("Unresolved version " + version + " for " + 
          details.getRequested().getGroup() + ":" + details.getRequested().getName() + 
          ". Will not use it as a version.");
      } else {
        project.getLogger().debug("Resolved version " + version + " for " + 
          details.getRequested().getGroup() + ":" + details.getRequested().getName());
        details.useVersion(version);
      }
    } else {
      project.getLogger().debug("No managed version for " + 
        details.getRequested().getGroup() + ":" + details.getRequested().getName() + 
        " --> using version " + details.getRequested().getVersion());
    }
  }
